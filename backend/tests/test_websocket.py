...
async def test_websocket_step_message():
    ...
    with client.websocket_connect("/ws") as websocket:
        # 发送step消息
        websocket.send_json({"type": "step", "content": "设未知数x"})
        # 接收hint响应
        response = websocket.receive_json()
...
async def test_websocket_error_report():
    ...
    with client.websocket_connect("/ws") as websocket:
        # 发送error_report消息
        websocket.send_json({"type": "error_report", "content": "测试错误报告"})
        # 接收acknowledge响应
        response = websocket.receive_json()
...
async def test_websocket_invalid_message():
    ...
    with client.websocket_connect("/ws") as websocket:
        # 发送无效消息（缺少type字段）
        websocket.send_json({"content": "无效消息"})
        # 接收error响应
        response = websocket.receive_json()
...
async def test_websocket_unknown_message_type():
    ...
    with client.websocket_connect("/ws") as websocket:
        # 发送未知类型消息
        websocket.send_json({"type": "unknown_type", "content": "未知类型消息"})
        # 接收error响应
        response = websocket.receive_json()
        assert response["type"] == "error"
...
async def test_health_check():
    ...
    assert response.json() == {"status": "ok", "message": "API服务正常运行"}
